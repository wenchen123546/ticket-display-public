...
function renderLineSettings() {
    ...
    Object.entries(cachedLine).forEach(([key, val]) => {
        ...
        if(canEdit) acts.appendChild(mk("button","btn-secondary",T.edit,{onclick:()=>{const nv=prompt(key+":",val||"");if(nv!==null)req("/api/admin/line-settings/save",{[key]:nv}).then(r=>{if(r){toast(T.saved,"success");loadLineSettings();}});}}));
...
socket.on("update", n => { $("number").textContent=n; if(checkPerm('perm_stats_view')) debouncedLoadStats(); });
