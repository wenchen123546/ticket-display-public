...
function renderAppointments(list) { 
    ...
    renderList("appointment-list-ui", list, a => {
        ...
        return mk("li","list-item",null,{},[mk("div","list-info",null,{},[mk("span","list-main-text",`${a.number} 號`,{style:"color:var(--primary);font-size:1rem;"}),mk("span","list-sub-text",`📅 ${new Date(a.scheduled_time).toLocaleString('zh-TW',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false})}`)]), acts]);
    }, "no_appt"); 
}
...
bind("btn-add-appt",async()=>{const n=$("appt-number").value,t=$("appt-time").value,ts=$("appt-time")._flatpickr?.selectedDates?.[0]?.getTime() ?? new Date(t.replace(' ','T')).getTime(); if(n&&t && await req("/api/appointment/add",{number:parseInt(n),timestamp:ts,timeStr:t})){toast(T.saved,"success");$("appt-number").value="";$("appt-time")._flatpickr?.clear();}});
