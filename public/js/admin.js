...
const mk = (t, c, txt, ev={}, ch=[]) => {
    ...
    const { isHtml, ...props } = ev;
    // 預設一律用 textContent；只有明確傳 isHtml:true 的靜態模板才用 innerHTML，避免暱稱 / 連結名稱等使用者資料造成 XSS
    if(txt!=null) e[isHtml?'innerHTML':'textContent']=String(txt);
    Object.entries(props).forEach(([k,v])=>k.startsWith('on')?e[k.toLowerCase()]=v:k==='style'?e.style.cssText=v:k.includes('-')?e.setAttribute(k,v):e[k]=v);
...
const updateLangUI = () => {
    ...
    $$('button[data-original-key]').forEach(b => !b.classList.contains('is-confirming') && (b.textContent = T[b.dataset.originalKey] ?? b.dataset.originalKey));
    
    if(checkPerm('perm_users_view')) loadUsers();
    if(checkPerm('perm_stats_view') || checkPerm('perm_logs_view')) loadStats();
    if(checkPerm('perm_booking_view')) { loadAppointments(); loadBookingConfig(); }
    if(checkPerm('perm_system_view')) loadPauseInfo();
    ...
    ["pause-resume","pause-reason"].forEach(id => $(id) && ($(id).disabled = !canSysEdit));
    if($("btn-save-pause")) $("btn-save-pause").style.display = canSysEdit ? "" : "none";
...
// Token 是 HttpOnly Cookie，JS 無法清除，必須呼叫後端銷毀 Session；只移除後台自己的 key，不影響同網域前台的票號 / 主題設定
let loggingOut = false;
const logout = async () => {
    if(loggingOut) return; loggingOut = true;
    try { await fetch("/logout", {method:"POST"}); } catch(e) {}
    ['callsys_user','callsys_role','callsys_nick'].forEach(k => localStorage.removeItem(k)); location.reload();
};

// ==========================================
// 閒置自動登出機制 (Idle Timeout)
// ==========================================
let idleTimer, lastIdleReset = 0;
const IDLE_TIMEOUT = 30 * 60 * 1000; // 預設 30 分鐘 (1800000 毫秒)

const resetIdleTimer = () => {
    // 節流：mousemove 觸發頻繁，5 秒內只重設一次
    const now = Date.now(); if(idleTimer && now - lastIdleReset < 5000) return; lastIdleReset = now;
...
// Session 檢查
// ==========================================
const applyPermVisibility = () => {
    $$('[data-perm]').forEach(el => {
        if (!['admin-card','nav-btn','card-wrapper'].some(c => el.classList.contains(c))) return;
        el.style.display = checkPerm(el.getAttribute('data-perm')) ? (el.classList.contains('admin-card') ? 'flex' : '') : 'none';
    });
...
const checkSession = async () => {
    ...
    if(uniqueUser) {
        // 以伺服器 Session 為準 (角色 / 暱稱可能已被變更)；401 時 req 會自動登出
        const me = await req("/api/admin/me");
        if(loggingOut) return;
        if(me) { uniqueUser = me.username; userRole = me.userRole; username = me.nickname; localStorage.setItem('callsys_user', uniqueUser); localStorage.setItem('callsys_role', userRole); localStorage.setItem('callsys_nick', username); }
...
function upgradeSystemModeUI() {
    const c = $("mode-switcher-group"); if(!c || c.querySelector('.segmented-control')) return;
    const radios = c.querySelectorAll('input[type="radio"]'); if(radios.length<2) return;
    const title = c.querySelector(':scope > label'), w = mk('div', 'segmented-control');
    radios.forEach(r => {
        const lbl = mk('label', 'segmented-option', T[r.value==='ticketing'?'mode_online':'mode_manual']||r.value, {onclick:(e)=>{ e.preventDefault(); if(!r.disabled && !r.checked){r.checked=true;r.dispatchEvent(new Event('change'));} updateSegmentedVisuals(w); }});
        lbl.dataset.i18n = r.value==='ticketing'?'mode_online':'mode_manual'; w.appendChild(lbl); lbl.appendChild(r);
    });
    // 先保留標題再清空 (原本清空後才查詢標題，導致「取號模式」標題消失)
    c.replaceChildren(...(title?[title]:[]), w); updateSegmentedVisuals(w);
}
...
async function loadFrontendTexts() {
    ...
    const keys = [
        ...
        {k:'counter_to', l:'櫃台提示 (預設: 請至 %s，%s 會換成櫃台名稱)'}
    ];
    ctr.innerHTML = "";
    const canEdit = checkPerm('perm_system_edit');
...
async function loadLineAutoReplies() {
    ...
    req("/api/admin/line-default-reply/get").then(r => {
        if(r && $("line-default-msg")) { $("line-default-msg").value = r.reply||""; $("line-default-msg").disabled = !canEdit; }
    });
...
async function loadLineSettings() {
    cachedLine = await req("/api/admin/line-settings/get");
    renderLineSettings();
}
function renderLineSettings() {
    if(!cachedLine || !$("line-settings-list-ui")) return;
    const canEdit = checkPerm('perm_line_edit');
    const ul = $("line-settings-list-ui"); ul.innerHTML = "";
    Object.entries(cachedLine).forEach(([key, val]) => {
        const acts = mk("div","list-actions");
        if(canEdit) acts.appendChild(mk("button","btn-secondary",T.edit,{onclick:()=>{const nv=prompt(`${key} (輸入新值，留白則不變更):`,"");if(nv&&nv.trim())req("/api/admin/line-settings/save",{[key]:nv}).then(r=>{if(r){toast(T.saved,"success");loadLineSettings();}});}}));
        ul.appendChild(mk("li","list-item",null,{},[mk("div","list-info",null,{},[mk("span","list-main-text",key),mk("span","list-sub-text",val?"••••"+val.slice(-4):"(未設定)")]),acts]));
    });
    if($("unlock-pwd-group")) $("unlock-pwd-group").style.display = canEdit ? "block" : "none";
    if(canEdit) req("/api/admin/line-settings/get-unlock-pass").then(r=>{ if(r && $("line-unlock-pwd")) $("line-unlock-pwd").value = r.password||""; });
}
...
const renderLogs = (logs, isInit) => {
    ...
    logs.forEach(l => {
        ...
        const match = l.match(/^\[(.*?)\] \[(.*?)\] (.*)$/);
        if(match) {
            li.appendChild(mk("span",null,`[${match[1]}]`,{style:"color:var(--text-light); margin-right:8px; font-family:'JetBrains Mono', monospace;"}));
            li.appendChild(mk("span",null,match[2],{style:"color:var(--primary); font-weight:bold; margin-right:8px;"}));
            li.appendChild(mk("span",null,match[3],{style:"color:var(--text-main);"}));
        } else {
            li.textContent = l;
        }
        frag.appendChild(li);
...
const debouncedLoadStats = () => { if(statsDebounceTimer) return; statsDebounceTimer = setTimeout(() => { statsDebounceTimer = null; loadStats(); }, 2000); };
...
socket.on("updateQueue", d => { 
    ...
    $("waiting-count").textContent = d.waiting ?? Math.max(0, d.issued - (d.max ?? d.current));
    if(checkPerm('perm_stats_view')) debouncedLoadStats(); 
});
...
// 過號名單：客人按「我回來了」的號碼標示時間並排到最前面
let passedList = [], passedBack = {}, passedBackReady = false;
const renderPassed = () => renderList("passed-list-ui", [...passedList].sort((a, b) => (passedBack[a] ? 0 : 1) - (passedBack[b] ? 0 : 1) || (passedBack[a]||0) - (passedBack[b]||0)), n => {
    const canEdit = checkPerm('perm_passed_edit'), back = passedBack[n];
    const acts = mk("div","list-actions",null,{},[]);
    if(canEdit) {
        acts.appendChild(mk("button","btn-secondary",T.recall,{onclick:()=>{if(confirm(T.msg_recall_confirm.replace('%s',n))) req("/api/control/recall-passed",withCounter({number:n}));}}));
        acts.appendChild((b=>{confirmBtn(b,T.del,()=>req("/api/passed/remove",{number:n}));return b;})(mk("button","btn-secondary",T.del)));
    }
    const info = mk("div","list-info",null,{},[mk("span","list-main-text",`${n} 號`,{style:"font-size:1rem;color:var(--primary);"}), back ? mk("span","passed-back-badge",`🙋 ${T.passed_back} ${new Date(back).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',hour12:false})}`) : null]);
    return mk("li",`list-item${back?' is-back':''}`,null,{},[info, acts]);
}, "empty");
socket.on("updatePassed", l => { passedList = l || []; renderPassed(); });
socket.on("updatePassedBack", b => {
    const next = b || {};
    if(passedBackReady && checkPerm('perm_passed_view')) Object.keys(next).filter(n => !passedBack[n]).forEach(n => toast(`🙋 ${n} ${T.msg_back}`, "info"));
    passedBack = next; passedBackReady = true; renderPassed();
});
...
socket.on("updatePauseInfo", p => fillPause(p));
...
function renderAppointments(list) { 
    ...
    ["appt-name","appt-phone"].forEach(id => $(id) && ($(id).disabled = !canEdit));
    ...
    renderList("appointment-list-ui", list, a => {
        ...
        const sub = mk("div","appt-sub",null,{},[
            mk("span",null,`📅 ${new Date(a.scheduled_time).toLocaleString('zh-TW',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false})}`),
            a.name ? mk("span",null,`👤 ${a.name}`) : null,
            a.phone ? mk("a",null,`📞 ${a.phone}`,{href:`tel:${a.phone}`}) : null,
            mk("span",null,a.source === 'online' ? T.source_online : T.source_admin)
        ]);
        return mk("li","list-item",null,{},[mk("div","list-info",null,{},[mk("span","list-main-text",`${a.number} 號`,{style:"color:var(--primary);font-size:1rem;"}), sub]), acts]);
    }, "no_appt"); 
}
...
async function loadRoles() {
    ...
    const thead = mk("thead",null,null,{},[mk("tr",null,null,{},[mk("th",null,"權限分組 / 角色"), ...roles.map(r=>mk("th",`th-role ${meta[r].c}`,`<div class="th-content"><span class="th-icon">${meta[r].icon}</span><span>${meta[r].l}</span></div>`,{isHtml:true}))])]);
...
async function loadStats() {
    try {
        ...
        if (d?.hourlyCounts) {
            ...
            renderList("stats-list-ui", d.history || [], h => mk("li","list-item",null,{},[(s=>{ s.append(`${new Date(h.timestamp).toLocaleTimeString('zh-TW', {hour:'2-digit',minute:'2-digit'})} - `, mk("b",null,h.number,{style:"color:var(--primary)"}), " ", mk("small",null,`(${h.operator})`,{style:"color:var(--text-sub)"})); return s; })(mk("span"))]), "no_logs");
        }
    } catch (e) { console.error(e); }
}

// 樂觀更新：req 失敗時回傳 null (不會 throw)，需依回傳值還原數字，否則畫面會停在錯誤號碼
// 叫號類 API 一律附上本機櫃台名稱 (存在瀏覽器，每台電腦各自設定)
const withCounter=(d={})=>({...d,counter:localStorage.getItem('callsys_counter')||''});
const act=(id,api,data={})=>$(id)?.addEventListener("click",async()=>{const n=api.includes('call')&&data.direction?$("number"):null,ov=n?.textContent; if(n){n.textContent=Math.max(0,parseInt(ov||0,10)+(data.direction==='next'?1:-1));n.style.opacity="0.6";} const r=await req(api,api.startsWith('/api/control/')?withCounter(data):data,$(id)); if(n){if(!r)n.textContent=ov;n.style.opacity="1";}}), bind=(id,fn)=>$(id)?.addEventListener("click",fn), adjCur=async d=>{const n=$("number"),c=parseInt(n.textContent||0,10),t=c+d;if(t>0){n.textContent=t;n.style.opacity="0.6";const r=await req("/api/control/set-call",withCounter({number:t}));if(r)toast(`${T.saved}: ${t}`,"success");else n.textContent=c;n.style.opacity="1";}};
...
bind("setNumber",async()=>{const n=$("manualNumber").value; if(n>0 && await req("/api/control/set-call",withCounter({number:n}))){$("manualNumber").value="";toast(T.saved,"success");}});
...
// 以瀏覽器當地時間換算成毫秒時間戳傳給後端，避免伺服器 (UTC) 解析字串造成 8 小時時差
bind("btn-add-appt",async()=>{const n=$("appt-number").value,t=$("appt-time").value,ts=$("appt-time")._flatpickr?.selectedDates?.[0]?.getTime() ?? new Date(t.replace(' ','T')).getTime(); if(n&&t && await req("/api/appointment/add",{number:parseInt(n),timestamp:ts,timeStr:t,name:$("appt-name").value,phone:$("appt-phone").value})){toast(T.saved,"success");$("appt-number").value=$("appt-name").value=$("appt-phone").value="";$("appt-time")._flatpickr?.clear();}});
bind("btn-save-roles",async()=>{
    ...
    if(await req("/api/admin/roles/update",{rolesConfig:c})){toast(T.saved,"success"); globalRoleConfig=c; applyPermVisibility();}
});
...
// 暫停服務說明
// ==========================================
const fillPause = p => { if($("pause-resume")) $("pause-resume").value = p?.resumeAt || ""; if($("pause-reason")) $("pause-reason").value = p?.reason || ""; };
async function loadPauseInfo() { if(!$("pause-resume") || !checkPerm('perm_system_view')) return; const p = await req("/api/admin/pause-info/get"); if(p) fillPause(p); }
bind("btn-save-pause", async () => { if(await req("/api/admin/pause-info/save", { resumeAt: $("pause-resume").value, reason: $("pause-reason").value }, $("btn-save-pause"))) toast(T.saved, "success"); });

// ==========================================
// 線上預約設定
// ==========================================
const BK_FIELDS = { daysAhead: "bk-days", start: "bk-start", end: "bk-end", slotMinutes: "bk-slot", capacity: "bk-cap", numberStart: "bk-numstart" };
async function loadBookingConfig() {
    if(!$("bk-enabled") || !checkPerm('perm_booking_view')) return;
    const c = await req("/api/admin/booking-config/get"); if(!c) return;
    const canEdit = checkPerm('perm_booking_edit');
    $("bk-enabled").checked = c.enabled; $("bk-enabled").disabled = !canEdit;
    Object.entries(BK_FIELDS).forEach(([k, id]) => { $(id).value = c[k]; $(id).disabled = !canEdit; });
    $("btn-save-booking-cfg").style.display = canEdit ? "" : "none";
}
bind("btn-save-booking-cfg", async () => {
    const body = { enabled: $("bk-enabled").checked }; Object.entries(BK_FIELDS).forEach(([k, id]) => body[k] = $(id).value);
    if(await req("/api/admin/booking-config/save", body, $("btn-save-booking-cfg"))) { toast(T.saved, "success"); loadBookingConfig(); }
});

// ==========================================
// 營運報表
// ==========================================
const twTodayStr = () => new Date().toLocaleDateString('en-CA', {timeZone:'Asia/Taipei'});
const shiftDay = (ds, n) => new Date(Date.parse(`${ds}T00:00:00Z`) + n * 864e5).toISOString().slice(0, 10);
const fmtPct = v => v == null ? '-' : `${Math.round(v * 1000) / 10}%`;
const fmtMin = v => v == null ? '-' : `${v} ${T.unit_min}`;
const fmtPeak = h => h == null ? '-' : `${String(h).padStart(2, '0')}:00`;
const setReportRange = p => { const t = twTodayStr(); $("report-to").value = t; $("report-from").value = p === 'month' ? `${t.slice(0, 8)}01` : shiftDay(t, -(Number(p) - 1)); };
async function runReport() {
    if(!$("report-body") || !checkPerm('perm_stats_view')) return;
    if(!$("report-from").value || !$("report-to").value) setReportRange(7);
    const d = await req("/api/admin/report", { from: $("report-from").value, to: $("report-to").value, group: $("report-group").value }, $("btn-report-run"));
    if(!d) return;
    const t = d.total;
    $("report-summary").replaceChildren(...[[T.th_issued, t.issued], [T.th_called, t.called], [T.th_wait, fmtMin(t.avgWaitMin)], [T.th_pass, fmtPct(t.passRate)], [T.th_cancel, fmtPct(t.cancelRate)], [T.th_peak, fmtPeak(t.peakHour)]].map(([label, v]) => mk("div", "report-tile", null, {}, [mk("div", "t", label), mk("div", "v", v)])));
    const rows = d.groups.filter(g => g.issued || g.called || g.cancelled);
    $("report-body").replaceChildren(...(rows.length
        ? rows.map(g => mk("tr", null, null, {}, [g.from === g.to ? g.from : `${g.from} ~ ${g.to}`, `${g.issued}${g.online ? ` (${T.online_short} ${g.online})` : ''}`, g.called, fmtMin(g.avgWaitMin), fmtMin(g.avgIntervalMin), fmtPct(g.passRate), fmtPct(g.cancelRate), fmtPeak(g.peakHour)].map(v => mk("td", null, v))))
        : [mk("tr", null, null, {}, [mk("td", null, T.report_empty, { colSpan: 8, style: "text-align:center;color:var(--text-sub);" })])]));
}
bind("btn-report-run", runReport);
$$('[data-report-preset]').forEach(b => b.addEventListener("click", () => { setReportRange(b.dataset.reportPreset); runReport(); }));
bind("btn-report-csv", async () => {
    if(!$("report-from").value || !$("report-to").value) setReportRange(7);
    const d = await req("/api/admin/export-csv", { from: $("report-from").value, to: $("report-to").value }, $("btn-report-csv"));
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // CSP 禁止 HTML inline 事件屬性 (onsubmit / oninput)，改在這裡綁定
    $("login-form")?.addEventListener("submit", e => e.preventDefault());
    ["manualNumber","manualIssuedNumber"].forEach(id => $(id)?.addEventListener("input", e => { if(!e.target.validity.valid) e.target.value=''; }));
    const ci=$("counter-name"); if(ci){ ci.value=localStorage.getItem('callsys_counter')||''; ci.addEventListener("input",()=>localStorage.setItem('callsys_counter',ci.value.trim())); }
    [ $("admin-lang-selector"), $("admin-lang-selector-mobile") ].forEach(sel => { if(sel){ sel.value=curLang; sel.addEventListener("change",e=>{curLang=e.target.value;localStorage.setItem('callsys_lang',curLang);$$(".lang-select").forEach(s=>s.value=curLang);updateLangUI();}); }});
    ...
    $$('.nav-btn').forEach(b => b.onclick = () => { $$('.nav-btn').forEach(x=>x.classList.remove('active')); b.classList.add('active'); $$('.section-group').forEach(s=>s.classList.remove('active')); const t=$(b.dataset.target); if(t){ t.classList.add('active'); if(b.dataset.target==='section-stats') { loadStats(); runReport(); } if(b.dataset.target==='section-settings'){ loadAppointments(); loadBookingConfig(); loadPauseInfo(); loadUsers(); if(checkPerm('perm_line_view')){cachedLine?renderLineSettings():loadLineSettings(); loadLineMessages(); loadLineAutoReplies(); loadLineSystemCommands();} } } });
    // 開關失敗時還原勾選狀態，避免畫面與伺服器不一致
    [["sound-toggle","/set-sound-enabled","enabled"],["public-toggle","/set-public-status","isPublic"],["ticketing-toggle","/set-ticketing-enabled","enabled"]].forEach(([id,url,key]) => $(id)?.addEventListener("change", async e => { const t=e.target; if(!(await req(url,{[key]:t.checked}))) t.checked=!t.checked; }));
    // 取消切換時要選回另一個選項 (原本直接取消勾選，導致兩個模式都未選取)
    $$('input[name="systemMode"]').forEach(r=>r.onchange=async()=>{ const other=[...$$('input[name="systemMode"]')].find(x=>x!==r); if(!confirm(T.confirm+" Switch Mode?") || !(await req("/set-system-mode",{mode:r.value}))) { r.checked=false; if(other) other.checked=true; } const w=document.querySelector('.segmented-control'); if(w) updateSegmentedVisuals(w); });
    document.addEventListener("keydown", e => { if(["INPUT","TEXTAREA"].includes(document.activeElement.tagName)){ if(e.key==="Enter"&&!e.shiftKey) {const m={"username-input":"login-button","password-input":"login-button","manualNumber":"setNumber","manualIssuedNumber":"setIssuedNumber","new-passed-number":"add-passed-btn"}; if(m[document.activeElement.id]) $(m[document.activeElement.id])?.click();} return;}
        // 快捷鍵只在已登入且位於現場控台時生效，並忽略組合鍵與下拉選單
        if($("admin-panel").style.display==="none" || !$("section-live")?.classList.contains("active") || e.ctrlKey || e.metaKey || e.altKey || document.activeElement.tagName==="SELECT") return;
