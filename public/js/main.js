...
socket.on("connect", () => { socket.emit('joinRoom', 'public'); clearTimeout(connTimer); $("status-bar").textContent=gt('conn'); $("status-bar").classList.remove("visible"); })
    ...
    .on("updateQueue", d => {
        ...
        if(el.textContent !== String(d.current)) {
            ...
            el.textContent = d.current; document.title = `${d.current} - Queue`;
        }
    ...
    .on("updateFeaturedContents", l => { const ct = $("featured-container"); if(!ct) return; ct.innerHTML = ""; (l||[]).forEach(c => { const a = d.createElement('a'); a.className = 'link-chip'; a.href = c.linkUrl; a.target = '_blank'; a.rel = 'noopener'; a.textContent = c.linkText; ct.appendChild(a); }); })
