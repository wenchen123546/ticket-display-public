...
// 增加 brand_title 預設字典
const i18n={"zh-TW":{brand_title:"即時叫號系統",cur:"目前叫號",iss:"已發至",online:"線上取號",help:"免排隊，手機領號",man_t:"號碼提醒",man_p:"輸入您的號碼開啟到號提醒",take:"立即取號",track:"追蹤",my:"我的號碼",ahead:"前方",wait:"⏳ 剩 %s 組",arr:"🎉 輪到您了！",pass:"⚠️ 已過號",p_list:"過號",none:"無",links:"精選連結",copy:"複製",sound:"音效",s_on:"開啟",s_off:"靜音",scan:"掃描追蹤",off:"連線中斷",ok:"取號成功",fail:"失敗",no_in:"請輸入號碼",cancel:"取消追蹤？",copied:"已複製",notice:"📢 ",q_left:"還剩 %s 組！",est:"約 %s 分",est_less:"< 1 分",est_at:"預計 %s 到號",just:"剛剛",ago:"%s 分前",conn:"已連線",retry:"連線中 (%s)...",wait_count:"等待中",sys_close:"⛔ 系統已暫停服務",sys_close_desc:"請稍候，我們將很快回來",recall_badge:"↩️ 過號重呼",counter_to:"請至 %s",recent:"最近叫號",cancel_ticket:"確定放棄此號碼？放棄後無法復原",has_ticket:"您已有號碼",cancelled:"已放棄號碼",im_back:"🙋 我已回到現場",im_back_done:"✅ 已通知櫃台，請留意叫號",im_back_fail:"此號碼目前不在過號名單",kiosk_start:"點一下開始叫號廣播",kiosk_start_desc:"瀏覽器需要點擊一次才能播放提示音與語音",resume_at:"預計 %s 恢復服務",booking_title:"線上預約",booking_desc:"選擇時段，到時優先叫號",booking_name:"稱呼（例如：王先生）",booking_phone:"手機號碼",booking_privacy:"手機號碼僅用於預約聯繫，服務結束 30 天後自動刪除",booking_submit:"確認預約",booking_mine:"您的預約",booking_track:"追蹤此號碼",booking_cancel:"取消預約",booking_cancel_confirm:"確定取消這筆預約？",booking_ok:"預約成功",booking_cancelled:"已取消預約",booking_left:"剩 %s",booking_full:"額滿",booking_no_slots:"這天已沒有可預約的時段",booking_status_pending:"已預約",booking_status_called:"已叫號",booking_wait:"📅 預約 %s，時間到會優先叫號",booking_pick:"請選擇時段並填寫資料"},"en":{brand_title:"Queue System",cur:"Now Serving",iss:"Issued",online:"Get Ticket",help:"Digital ticket & notify",man_t:"Number Alert",man_p:"Enter number to get alerted",take:"Get Ticket",track:"Track",my:"Your #",ahead:"Ahead",wait:"⏳ %s groups",arr:"🎉 Your Turn!",pass:"⚠️ Passed",p_list:"Passed",none:"None",links:"Links",copy:"Copy",sound:"Sound",s_on:"On",s_off:"Mute",scan:"Scan",off:"Offline",ok:"Success",fail:"Failed",no_in:"Enter #",cancel:"Stop tracking?",copied:"Copied",notice:"📢 ",q_left:"%s groups left!",est:"~%s min",est_less:"< 1 min",est_at:"ETA %s",just:"Now",ago:"%s m ago",conn:"Online",retry:"Retry (%s)...",wait_count:"Waiting",sys_close:"⛔ System Paused",sys_close_desc:"Please wait, we will be back soon",recall_badge:"↩️ Recalled",counter_to:"Please go to %s",recent:"Recent",cancel_ticket:"Give up this ticket? This cannot be undone",has_ticket:"You already have a ticket",cancelled:"Ticket cancelled",im_back:"🙋 I'm back",im_back_done:"✅ Staff notified, please watch for your number",im_back_fail:"This number is not on the passed list",kiosk_start:"Tap to start announcements",kiosk_start_desc:"Browsers need one tap before playing sound and voice",resume_at:"Expected back at %s",booking_title:"Book a Time",booking_desc:"Pick a slot and get called first",booking_name:"Name",booking_phone:"Mobile number",booking_privacy:"Your number is only used for this booking and deleted 30 days after service",booking_submit:"Confirm Booking",booking_mine:"Your Booking",booking_track:"Track this number",booking_cancel:"Cancel booking",booking_cancel_confirm:"Cancel this booking?",booking_ok:"Booked",booking_cancelled:"Booking cancelled",booking_left:"%s left",booking_full:"Full",booking_no_slots:"No slots left on this day",booking_status_pending:"Booked",booking_status_called:"Called",booking_wait:"📅 Booked for %s, you'll be called first",booking_pick:"Pick a slot and fill in your details"}};
...
// passedNums：過號名單；calledSeen：本次連線看過被叫到的號碼 (預約號不在一般進度內，用來判斷是否已叫過)
let passedNums = [], calledSeen = new Set(), pauseInfo = {}, bookingCfg = null, bookingDate = null, bookingSlot = null, myBooking = null;
...
const clearTicket = () => { ['callsys_ticket', 'callsys_ticket_src', 'callsys_ticket_time', 'callsys_back'].forEach(k => ls.removeItem(k)); myTicket = null; lastDiff = null; disablePush(); renderMode(); resetTakeBtn(); };
...
// 過號客人：顯示「我已回到現場」按鈕；已回報過顯示提示 (號碼離開過號名單時清除紀錄)
const renderBack = diff => {
    const b = $("btn-im-back"), done = $("im-back-done"); if(!b || !done) return;
    const passed = diff === -1 && passedNums.includes(myTicket), sent = passed && ls.getItem('callsys_back') === String(myTicket);
    if(!passed && ls.getItem('callsys_back')) ls.removeItem('callsys_back');
    b.style.display = passed && !sent ? '' : 'none'; done.style.display = sent ? '' : 'none';
};

// 暫停畫面：原因與預計恢復時間
const renderPause = () => {
    const e = $("overlay-resume"); if(!e) return;
    const parts = [pauseInfo.reason, pauseInfo.resumeAt && gt('resume_at').replace('%s', pauseInfo.resumeAt)].filter(Boolean);
    e.textContent = parts.join(' · '); e.style.display = parts.length ? '' : 'none';
};

// 大螢幕模式：瀏覽器規定要先點擊一次才能播放音效與語音，開機後顯示遮罩提醒
const showKioskStart = () => {
    const ov = el('div', {id:'kiosk-start', className:'kiosk-start'}, el('div', {className:'kiosk-start-icon', textContent:'🔊'}), el('h2', {textContent:gt('kiosk_start')}), el('p', {textContent:gt('kiosk_start_desc')}));
    on(ov, 'click', () => {
        unlockAudio(); updateMuteUI(false);
...
const uiLocale = () => lang === 'en' ? 'en-US' : 'zh-TW';
const fmtDateChip = ds => new Date(`${ds}T12:00:00+08:00`).toLocaleDateString(uiLocale(), {timeZone:'Asia/Taipei', month:'numeric', day:'numeric', weekday:'short'});
const fmtSlotTime = ts => new Date(ts).toLocaleString(uiLocale(), {timeZone:'Asia/Taipei', month:'numeric', day:'numeric', weekday:'short', hour:'2-digit', minute:'2-digit', hour12:false});
const updateBookingSubmit = () => { const b = $("btn-booking-submit"); if(b) b.disabled = !(bookingSlot && $("booking-name").value.trim() && $("booking-phone").value.trim()); };
const loadSlots = async () => {
    const box = $("booking-slots"); if(!box || !bookingDate) return;
    bookingSlot = null; updateBookingSubmit();
    try {
        const slots = (await getJSON(`/api/booking/slots?date=${encodeURIComponent(bookingDate)}`)).slots || [];
        box.replaceChildren(...(slots.length ? slots.map(s => {
            const full = s.remaining <= 0, btn = el('button', {type:'button', className:'booking-slot', disabled:full}, el('span', {className:'slot-time', textContent:s.time}), el('span', {className:'slot-left', textContent: full ? gt('booking_full') : gt('booking_left').replace('%s', s.remaining)}));
            on(btn, 'click', () => { bookingSlot = s.ts; $$('.booking-slot').forEach(x => x.classList.toggle('active', x === btn)); updateBookingSubmit(); });
            return btn;
        }) : [el('p', {className:'booking-empty', textContent:gt('booking_no_slots')})]));
    } catch(e) { box.replaceChildren(); }
};
const renderBooking = () => {
    const sec = $("booking-section"); if(!sec) return;
    const enabled = !!bookingCfg?.enabled;
    sec.style.display = (enabled || myBooking) ? '' : 'none';
    $("booking-form").style.display = myBooking || !enabled ? 'none' : '';
    $("booking-mine").style.display = myBooking ? '' : 'none';
    if(myBooking) {
        $("booking-mine-num").textContent = myBooking.number;
        $("booking-mine-time").textContent = fmtSlotTime(myBooking.scheduled_time);
        $("booking-mine-name").textContent = `${myBooking.name} · ${myBooking.phone}`;
        $("booking-mine-status").textContent = gt(`booking_status_${myBooking.status}`) || myBooking.status;
        // 預約當天才能追蹤 (號碼每天重置)
        $("btn-booking-track").style.display = myBooking.status === 'pending' && myBooking.date_str === twToday() && myTicket !== myBooking.number ? '' : 'none';
        $("btn-booking-cancel").style.display = myBooking.status === 'pending' ? '' : 'none';
    } else if(enabled) {
        const dates = bookingCfg.dates || [];
        if(!dates.includes(bookingDate)) { bookingDate = dates[0]; loadSlots(); }
        $("booking-dates").replaceChildren(...dates.map(ds => { const btn = el('button', {type:'button', className:`booking-date${ds === bookingDate ? ' active' : ''}`, textContent:fmtDateChip(ds)}); on(btn, 'click', () => { if(bookingDate === ds) return; bookingDate = ds; renderBooking(); loadSlots(); }); return btn; }));
    }
};
const loadBooking = async () => {
    try {
        const [cfg, mine] = await Promise.all([getJSON('/api/booking/config'), getJSON('/api/booking/mine')]);
        bookingCfg = cfg; myBooking = mine.booking && ['pending', 'called'].includes(mine.booking.status) ? mine.booking : null;
        renderBooking();
    } catch(e) {}
};
...
const applyText = () => {
    ...
    $$('[data-i18n]').forEach(e => { const k=map[e.dataset.i18n]||e.dataset.i18n; if(gt(k)) e.textContent=gt(k); });
    $$('[data-i18n-ph]').forEach(e => { if(gt(e.dataset.i18nPh)) e.placeholder = gt(e.dataset.i18nPh); });
    renderBooking(); renderPause();
...
const updateTicket = (curr) => {
    ...
    const src = ls.getItem('callsys_ticket_src');
    let diff;
    if (myTicket === curr) diff = 0;
    else if (src === 'booking') diff = calledSeen.has(myTicket) ? -1 : -2; // 預約號不在一般進度內，改顯示預約時段
    ...
    const bookingTime = new Date(Number(ls.getItem('callsys_ticket_time'))||0).toLocaleTimeString('zh-TW', {timeZone:'Asia/Taipei', hour:'2-digit', minute:'2-digit', hour12:false});
    $("ticket-status-text").textContent = diff > 0 ? gt('wait').replace("%s",diff) : diff===0 ? gt('arr') : diff===-2 ? gt('booking_wait').replace("%s", bookingTime) : gt('pass');
    renderBack(diff);
...
socket.on("connect", () => { clearTimeout(connTimer); $("status-bar").textContent=gt('conn'); $("status-bar").classList.remove("visible"); })
    ...
    .on("updateQueue", q => {
        ...
        (q.recent||[]).forEach(x => calledSeen.add(x.n));
        // 預約號不適用「過期」判斷 (當天開始時發號數為 0，但預約號仍有效)
        if(myTicket && ls.getItem('callsys_ticket_src') !== 'booking' && ((q.issued===0 && myTicket>5) || (myTicket < currentMax-20))) { clearTicket(); toast("票號已過期或系統重置", "info"); }
    ...
    .on("updatePublicStatus", b => {
        ...
        if(!ov) {
            ov = d.body.appendChild(el('div', {id:"closed-overlay"}, el('div', {style:'text-align:center;'},
                ...
                el('p', {id:'overlay-resume', className:'overlay-resume'}))));
    ...
    .on("updatePassed", l => { passedNums = (l||[]).map(Number); if(myTicket) updateTicket(parseInt($("number").textContent)||0); })
    .on("updatePauseInfo", p => { pauseInfo = p || {}; renderPause(); })
    .on("updateBookingConfig", () => loadBooking())
...
d.addEventListener("DOMContentLoaded", () => {
    if(isKioskMode()) { d.body.classList.add('kiosk-mode'); toggleWakeLock(true); showKioskStart(); }
    ...
    on($("btn-im-back"), "click", async function() {
        if(!myTicket) return;
        this.disabled = true;
        try {
            const r = await postJSON('/api/ticket/back', {number: myTicket}).then(x => x.json());
            if(r.notified) { ls.setItem('callsys_back', String(myTicket)); toast(gt('im_back_done'), "success"); } else toast(r.error || gt('im_back_fail'), "error");
        } catch(e) { toast(gt('off'), "error"); } finally { this.disabled = false; renderBack(lastDiff); }
    });

    ["booking-name", "booking-phone"].forEach(id => on($(id), "input", updateBookingSubmit));
    on($("btn-booking-submit"), "click", async function() {
        if(!bookingSlot) return toast(gt('booking_pick'), "error");
        this.disabled = true;
        try {
            const r = await postJSON('/api/booking/create', { ts: bookingSlot, name: $("booking-name").value, phone: $("booking-phone").value }).then(x => x.json());
            if(r.booking) { myBooking = r.booking; $("booking-name").value = $("booking-phone").value = ""; toast(gt('booking_ok'), "success"); renderBooking(); }
            else { toast(r.error || gt('fail'), "error"); loadSlots(); }
        } catch(e) { toast(gt('off'), "error"); } finally { updateBookingSubmit(); }
    });
    on($("btn-booking-cancel"), "click", async () => {
        if(!myBooking || !confirm(gt('booking_cancel_confirm'))) return;
        const num = myBooking.number;
        try {
            const r = await postJSON('/api/booking/cancel', {}).then(x => x.json());
            if(r.cancelled) { if(myTicket === num && ls.getItem('callsys_ticket_src') === 'booking') clearTicket(); myBooking = null; toast(gt('booking_cancelled'), "info"); }
            await loadBooking();
        } catch(e) { toast(gt('off'), "error"); }
    });
    on($("btn-booking-track"), "click", () => {
        if(!myBooking) return;
        const permP = askNotify(); ls.setItem('callsys_ticket_time', myBooking.scheduled_time);
        setTicket(myBooking.number, 'booking'); enablePush(myBooking.number, permP); renderBooking();
    });
    loadBooking();
});
