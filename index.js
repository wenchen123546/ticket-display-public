...
const sanitize = v => typeof v === 'string' ? v.replace(/[<>]/g, c => c === '<' ? '&lt;' : '&gt;') : v;
...
app.post("/api/passed/add", auth, perm('perm_passed_edit'), H(async r => { const n = validateNum(r.body.number, 1); if(n !== null) { await redis.zadd(KEYS.PASSED, n, n); await redis.hincrby(`${KEYS.HOURLY}${getTWTime().dateStr}`, `${getTWTime().hour}_p`, 1); io.emit("updatePassed", (await redis.zrange(KEYS.PASSED,0,-1)).map(Number)); addLog(r.user.nickname, `➕ 手動過號 ${n}`); } }));
...
['SIGTERM','SIGINT'].forEach(sig => process.on(sig, () => {
    ...
    if(dbQueue.length) { const batch = [...dbQueue]; dbQueue.length = 0; db.serialize(() => { db.run("BEGIN TRANSACTION"); const s = db.prepare("INSERT INTO history (date_str, timestamp, number, action, operator, wait_time_min) VALUES (?, ?, ?, ?, ?, ?)"); batch.forEach(r => s.run([r.dateStr, r.timestamp, r.number, r.action, r.operator, r.wait_time_min])); s.finalize(); db.run("COMMIT"); }); }
    server.close(() => { db.close(); redis.disconnect(); console.log('👋 Server closed.'); process.exit(0); });
